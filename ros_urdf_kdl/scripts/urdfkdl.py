...
def _toKdlInertia(i):
    ...
    inertia = i.inertia
    return origin.M * kdl.RigidBodyInertia(i.mass, origin.p, kdl.RotationalInertia(inertia.ixx, inertia.iyy, inertia.izz, inertia.ixy, inertia.ixz, inertia.iyz))
...
def _toKdlJoint(jnt):
    ...
    # Ensure the joint type is handled correctly
    if jnt.type not in type_map:
        raise ValueError(f"Unknown joint type: {jnt.type}")
...
def treeFromUrdfModel(robot_model, quiet=False):
    ...
    print("The root link is %s" % root.name)
